...
class TestIsolationConfig:
    ...
    target_app_package: Optional[str] = None  # 目标应用包名
    target_app_activity: Optional[str] = None  # 目标应用 Activity
    clear_app_data: bool = False  # 清除应用数据
    dismiss_dialogs: bool = True  # 自动关闭系统弹窗
...
class AndroidAgent(Agent[AndroidDevice]):
    ...
    Additionally provides Android-specific convenience methods:
    - launch_app: Launch Android app by package name
    - stop_app: Stop Android app by package name
    ...
    # Type hint for the interface to be AndroidDevice
    ...
    def __init__(
        ...
        # Store typed reference to device for Android-specific operations
    ...
    async def create(
        ...
        Args:
            ...
            locate_config: Locate configuration
            isolation_config: Test isolation configuration
            wait_config: Wait configuration
    ...
    async def reset_state(
        ...
        try:
            ...
            # 4. 清除应用数据（可选）
            if cfg.clear_app_data and cfg.target_app_package:
                await self._clear_app_data(cfg.target_app_package)
            
            # 5. 如果指定了目标应用，启动它
            if cfg.target_app_package:
                await self.launch_app(cfg.target_app_package, cfg.target_app_activity)
            ...
            if cfg.wait_for_idle:
                await self._wait_for_device_idle()
    ...
    async def _close_all_background_apps(self) -> None:
        ...
        try:
            ...
            # 尝试点击"清除全部"按钮
            try:
                await self.ai_action("点击清除全部或关闭所有应用按钮")
            except Exception:
                # 如果没有清除全部按钮，尝试逐个上滑关闭
                screen_size = self._android_device._screen_size
                if screen_size:
    ...
    async def _clear_notifications(self) -> None:
        ...
        try:
            ...
            if screen_size:
                center_x = screen_size.width / 2
                await self.swipe(center_x, 0, center_x, 500, duration=300)
                await asyncio.sleep(0.5)
    ...
    async def _clear_app_data(self, package_name: str) -> None:
        """清除应用数据
        ...
        """
        try:
            await self._android_device._run_adb_command([
                "-s", self._android_device.device_id, 
    ...
    async def _wait_for_device_idle(self, timeout_ms: int = 5000) -> None:
        """等待设备空闲
        ...
        try:
            ...
            logger.debug(f"Wait for idle check: {e}")
    ...
    async def setup_test(
        ...
        Args:
            app_package: 要启动的应用包名
            app_activity: 要启动的 Activity
            reset: 是否执行状态重置
            
        Example:
            async def test_login():
                await agent.setup_test(app_package="com.example.app")
                # 执行测试步骤...
        ...
        if reset:
            await self.reset_state(TestIsolationConfig(
                ...
                target_app_activity=app_activity
            ))
        elif app_package:
            await self.launch_app(app_package, app_activity)
    ...
    async def test_case(
        ...
        try:
            await self.setup_test(app_package=app_package, app_activity=app_activity, reset=reset)
            yield self
        except Exception as e:
    ...
    async def dismiss_system_dialog(self) -> bool:
        """尝试关闭系统弹窗
        ...
        try:
            ...
            logger.debug(f"No system dialog to dismiss: {e}")
            return False
    ...
    async def ai_input(
        ...
        if result.element:
            ...
            if clear_first:
                await self._android_device.clear_text()
                await asyncio.sleep(0.1)
            
            await self._android_device.input_text(text)
            ...
            # 提交
            if submit_after:
                await self._android_device.key_event("KEYCODE_ENTER")
        else:
    ...
    async def ai_scroll_to_element(
        ...
        for i in range(max_scrolls):
            ...
            # 滚动
            await self._android_device.scroll(direction)
            await asyncio.sleep(0.5)
